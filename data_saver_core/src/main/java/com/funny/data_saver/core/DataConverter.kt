...
inline fun <reified T : Any> registerTypeConverters(noinline save: ((T)->String)? = null, noinline restore: ((String)->T)? = null){
    save?.let { typeSaveConverters[T::class.java] = it as (Any) -> String }
    restore?.let { typeRestoreConverters[T::class.java] = it }
}

@Deprecated("Use the function with both save and restore instead.", replaceWith = ReplaceWith("registerTypeConverters(noinline save: ((T)->String)? = null, noinline restore: ((String)->T)? = null)"))
...
fun convertListToString(list: List<*>): String{
    ...
    if (sb.length > DataSaverConfig.LIST_SEPARATOR.length + 2) sb.delete(sb.length-DataSaverConfig.LIST_SEPARATOR.length, sb.length)
    sb.append("]")
    return sb.toString()
}
...
inline fun <reified T> convertStringToList(str: String) : List<T> {
    ...
    restoreConverter ?: error("Unable to restore $str: ${T::class.java} is not supported, please call [registerTypeConverters] at first!")
...
private fun unsupportedType(data: Any): Nothing =
    error("Unable to save data: type of $data (class: ${data::class.java} is not supported, please call [registerTypeConverters] at first!")
